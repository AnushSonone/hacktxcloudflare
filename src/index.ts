...
export class RaceDO {
  ...
  private startRace(): void {
    ...
    this.isRunning = true;
    this.raceStartTime = Date.now();
    console.log("🏁 Race simulation loop STARTED (20Hz)");
  ...
  private stopRace(): void {
    ...
    if (this.intervalId !== null) {
      clearInterval(this.intervalId);
      this.intervalId = null;
      console.log("🛑 Race simulation loop STOPPED");
    }
  }
    ...
    if (this.previousOrder.length > 0) {
      for (let i = 0; i < Math.min(currentOrder.length, this.previousOrder.length); i++) {
        if (currentOrder[i] !== this.previousOrder[i]) {
          console.log(`overtake: ${currentOrder[i]} passed ${this.previousOrder[i]} at raceTime=${this.raceTime.toFixed(2)}s`);
          break;
        }
      }
    }
  ...
  private handleWebSocket(ws: WebSocket): void {
    ...
    console.log(`✅ WebSocket client CONNECTED. Total clients: ${this.sessions.size}`);

    // Send session bootstrap message
    ws.send(JSON.stringify(this.sessionData));
    console.log("📤 Sent SESSION bootstrap message to client");
    ...
    ws.addEventListener("close", () => {
      this.sessions.delete(ws);
      console.log(`🔌 WebSocket client DISCONNECTED. Total clients: ${this.sessions.size}`);
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/ask") {
      ...
      if (latestTick === null) {
        ...
        answer = answerQuestion(q, latestTick);
